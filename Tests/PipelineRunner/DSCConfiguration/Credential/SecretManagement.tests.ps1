...
result-shape handling without SecretManagement installed.
#>
Describe "SecretManagement Function Tests" -Tag Unit, PipelineRunner, Credential {
    ...
    Context "with the SecretManagement module available" {
        ...
        It "Omits -Vault from Get-Secret when not supplied" {
            Mock -CommandName Get-Secret -MockWith { return $null }
            { & $script:SecretManagementPath -Context @{ Name = 'MySecret' } } | Should -Throw
            Assert-MockCalled -CommandName Get-Secret -ParameterFilter { $null -eq $Vault } -Exactly 1 -Scope It
        }
    }
}
