Describe "DscV2 Function Tests" -Tag Unit, PipelineRunner, Engine {
    ...
    BeforeAll {
        ...
        # PSDesiredStateConfiguration 2.x removed it - but the CimSession FALLBACK below has
        # to mock a call that passes one, and the engine probes the resolved command's
        # parameters before using it. Pester's Mock inherits the *original* target command's
        # parameter metadata at the time it is first mocked, so this stub must be defined -
        # and the first Mock call made against it - before that first Mock call.
    ...
    It "Defaults RebootRequired to false when the backend does not report it" {
        Mock -CommandName Invoke-DscResource -MockWith {
    ...
    It "Surfaces RebootRequired when the Set backend reports it" {
        Mock -CommandName Invoke-DscResource -MockWith {
            return [pscustomobject]@{ RebootRequired = $true }
        }

        $result = & $script:DscV2Path -Context @{ Method = 'Set'; ModuleName = 'Mod'; Name = 'Res'; Property = @{} }
    ...
    Context "Remote-target execution (#57 §4)" {
        ...
        It "Runs the evaluation over the PSSession when the target resolved one" {
            ...
            # Invoke-DscResource runs locally there.
            $Global:DscV2CapturedCalls.Clear()
