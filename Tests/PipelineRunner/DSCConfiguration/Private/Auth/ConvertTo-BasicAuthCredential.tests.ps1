...
Describe 'ConvertTo-BasicAuthCredential Function Tests' -Tag Unit, PipelineRunner, Auth {

    BeforeAll {

        # Load the functions to test
        $preParseFilePath = (Get-FunctionPath 'ConvertTo-BasicAuthCredential.ps1').FullName
        . $preParseFilePath
    ...
    Context 'Test-BasicAuthCredential' {
        ...
        It 'Should return false for a value whose length is not a multiple of four' {
            Test-BasicAuthCredential -Value 'abc' | Should -BeFalse
        }

        It 'Should return false for a value containing characters outside the base64 alphabet' {
            Test-BasicAuthCredential -Value 'abc!' | Should -BeFalse
        }

        It 'Should return false when the decoded bytes are not printable ASCII' {
            $encoded = [Convert]::ToBase64String(@(0, 1, 2, 3))
