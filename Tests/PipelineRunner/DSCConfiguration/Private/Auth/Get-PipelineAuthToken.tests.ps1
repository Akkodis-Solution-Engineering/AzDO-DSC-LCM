...
Describe "Get-PipelineAuthToken Function Tests" -Tag Unit, PipelineRunner, Auth {

    BeforeAll {

        # Load the functions to test
        # Unprotect-SecureString is loaded so the tests can assert on the resolved plaintext;
        # it is a module Private function normally already in scope when this file runs
    ...
    Context "Environment fallback" {
        ...
        It "Returns null when the fallback environment variable name is blank" {
            Get-PipelineAuthToken -FallbackEnvName '' | Should -BeNullOrEmpty
        }
    }
}
