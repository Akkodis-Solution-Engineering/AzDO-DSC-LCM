...
Describe "Get-PipelineRunnerSetting Function Tests" -Tag Unit, PipelineRunner, Configuration {

    BeforeAll {

        # Load the functions to test
        $preParseFilePath = (Get-FunctionPath 'Get-PipelineRunnerSetting.ps1').FullName
        . $preParseFilePath
    ...
    Context "Definition file resolution" {
        ...
        It "Reads the default Datum.yml file name when none is supplied" {
            Mock -CommandName Test-Path -MockWith { return $true }
            Mock -CommandName ConvertFrom-Yaml -MockWith { return @{ PipelineRunnerSettings = @{ Engine = 'DscV2' } } }

            $null = Get-PipelineRunnerSetting -ConfigurationDirectory 'TestDir'

            Assert-MockCalled -CommandName Test-Path -Exactly 1 -Scope It -ParameterFilter {
                $LiteralPath -like '*Datum.yml'
            }
        }
    ...
    Context "Settings extraction" {
        ...
        It "Returns null when the document is a scalar" {
            Mock -CommandName ConvertFrom-Yaml -MockWith { return 'just a string' }
