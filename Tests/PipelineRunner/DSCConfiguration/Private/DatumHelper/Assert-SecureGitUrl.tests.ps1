...
Describe 'Assert-SecureGitUrl Function Tests' -Tag Unit, PipelineRunner, DatumHelper {

    BeforeAll {

        # Load the functions to test
        $preParseFilePath = (Get-FunctionPath 'Assert-SecureGitUrl.ps1').FullName
        . $preParseFilePath
    ...
    Context 'Rejected transports' {
        ...
        It 'Should reject a whitespace-only URL' {
            { Assert-SecureGitUrl -Url '   ' } | Should -Throw -ExpectedMessage '*No repository URL*'
        }
    }
}
