Describe "Get-PipelineRunnerConfigurationMode Function Tests" -Tag Unit, PipelineRunner, Configuration {

    BeforeAll {

        # Load the functions to test
        $preParseFilePath = (Get-FunctionPath 'Get-PipelineRunnerConfigurationMode.ps1').FullName
    ...
    Context "When the top-level ConfigurationMode is 'ApplyOnly', 'Audit', or 'Enforce'" {
        ...
        It "should return the ConfigurationMode as ApplyOnly" {
            $result = Get-PipelineRunnerConfigurationMode -DatumConfigurationMode $DatumConfigurationMode
    ...
    Context "When ChangeWindows include a DaysOfWeek constraint" {
        ...
        It "should default to Audit when time matches but no window's DaysOfWeek includes the current day" {
            ...
            $result = Get-PipelineRunnerConfigurationMode -DatumConfigurationMode $dayOnlyConfig
            $result | Should -Be 'Audit'
        }
        ...
        It "should match a window with no DaysOfWeek constraint on any day" {
            ...
            $result = Get-PipelineRunnerConfigurationMode -DatumConfigurationMode $noDayConfig
            $result | Should -Be 'Enforce'
        }
        ...
        It "should match DaysOfWeek case-insensitively" {
            ...
            $result = Get-PipelineRunnerConfigurationMode -DatumConfigurationMode $mixedCaseConfig
            $result | Should -Be 'Enforce'
        }
