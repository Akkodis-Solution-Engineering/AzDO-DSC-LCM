...
Describe "Merge-DscRunnerResult Function Tests" -Tag Unit, PipelineRunner, Runner {

    BeforeAll {
        $preParseFilePath = (Get-FunctionPath 'Merge-DscRunnerResult.ps1').FullName
        . $preParseFilePath
    ...
    Context "aggregation" {
        ...
        It "drops null entries from the result collection" {
            $summary = Merge-DscRunnerResult -Result @($null, (New-RunnerResult), $null)
            $summary.TotalConfigurations | Should -Be 1
        }
        ...
        It "reports PartialSuccess when a configuration skips resources without stopping" {
            $results = @(
                (New-RunnerResult -File 'a.yml' -Pass 1 -Skip 1)
            )
        ...
        It "ranks Aborted above Failed when both occur in the same run" {
            $results = @(
                (New-RunnerResult -File 'a.yml' -Pass 0 -Fail 1)
                (New-RunnerResult -File 'b.yml' -Status 'AbortedByException' -Pass 0)
            )
    ...
    Context "report output" {
        ...
        It "writes an aggregate report.json when a ReportPath is supplied" {
            Merge-DscRunnerResult -Result @((New-RunnerResult)) -ReportPath 'TestDrive:\Reports' | Out-Null
