...
#>
Describe "SSH Function Tests" -Tag Unit, PipelineRunner, Target {
    ...
    It "Opens a PSSession over SSH when no Engine is supplied" {
        # Only DscV2 fails fast; an unset/other Engine value falls through to New-PSSession.
        $result = & $script:SSHPath -Context @{ ComputerName = 'node01' }
    ...
    It "Omits UserName and KeyFilePath from New-PSSession when not supplied" {
        $null = & $script:SSHPath -Context @{ ComputerName = 'node01'; Engine = 'DscV3' }

        Assert-MockCalled -CommandName New-PSSession -ParameterFilter {
            $null -eq $UserName -and $null -eq $KeyFilePath
        } -Exactly 1 -Scope It
    }
}
