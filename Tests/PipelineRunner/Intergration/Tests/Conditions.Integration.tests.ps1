Describe "preCondition/postCondition through a real runner pass" -Tag Integration, HostedIntegration {
    ...
    #
    #   * the condition string surviving the real file parse            (Start-DscRunner.ps1)
    #   * `variables()` reading what the real Set-Variables rule wrote
    #     from the file's own `variables` block                         (Start-DscRunner.ps1)
    #   * Assert-SafeConditionExpression + ConvertTo-NormalizedConditionExpression
    #     validating and rewriting the same text that is then executed  (Start-DscRunner.ps1)
    #   * `result()` returning the real [DscMethodResult] the engine seam built
    #     from the engine's own output                                  (Start-DscRunner.ps1)
    #   * `stopProcessing()` setting the flag the runner reads one resource later,
    #     in the order the real Sort-DependsOn produced                 (Start-DscRunner.ps1)
    #
    # As in NotifyUsing.Integration.tests.ps1, only the rule LOADERS are substituted (their
    ...
    # rule scripts under "Pipeline Rules/Custom", so ordering is genuinely exercised. This
    # repo's class-based configuration parsing chain
    # (DSCConfigurationFile/DSC_Resource/.../ConvertTo-PipelineTask) is dot-sourced for real
    # below, so [DSCConfigurationFile]::New() inside Start-DscRunner genuinely parses the JSON
    # fixtures this suite writes to TestDrive. The engine is an inline -EngineAction, so this
...
'@
            ...
            # preCondition-evaluation failure unless -ContinueOnError is passed, halting every
            # subsequent resource in the file (not just the offending one). -ContinueOnError is
            # what keeps the failure scoped to 'Unsafe' so 'Always' still runs, matching this
            # test's own title/intent.
            $result = Start-DscRunner -FilePath $config -ConfigurationMode 'ApplyOnly' -DSCCompositeResourcePath $script:CompositeResourcePath -EngineAction $engine -ContinueOnError -ErrorAction SilentlyContinue
...
'@
            ...
            # See the -ContinueOnError note above: without it, 'Always' would be skipped by the
            # StopTaskProcessing cascade rather than genuinely proving the failure is scoped to
            # 'Boundary'.
            $result = Start-DscRunner -FilePath $config -ConfigurationMode 'ApplyOnly' -DSCCompositeResourcePath $script:CompositeResourcePath -EngineAction $engine -ContinueOnError -ErrorAction SilentlyContinue
...
'@
            ...
            $null = Start-DscRunner -FilePath (New-ConfigFile -Name 'stop-leak-first.json' -Json $stopJson) -ConfigurationMode 'ApplyOnly' -DSCCompositeResourcePath $script:CompositeResourcePath -EngineAction $engine
            $script:StopTaskProcessing | Should -BeTrue

            $second = Start-DscRunner -FilePath (New-ConfigFile -Name 'stop-leak-second.json' -Json $plainJson) -ConfigurationMode 'ApplyOnly' -DSCCompositeResourcePath $script:CompositeResourcePath -EngineAction $engine
