Describe "DSC v2 environment lifecycle through a real Invoke-DscResource" -Tag Integration, DscV2SelfHosted {
    ...
    # (the PipelineRunnerFile test fixture under Tests/Fixtures/DscV2), so this exercises actual
    # PowerShell DSC on the host.
    #
    ...
    #
    # Everything is real: the compiled configuration (parsed through this repo's real
    # DSCConfigurationFile/DSC_Resource class chain), dependency ordering (Sort-DependsOn),
    # variable seeding/expansion, per-resource conditions, the engine seam and its
    # [DscMethodResult] contract, Invoke-DscResource itself, and the postExecutionScript /
    # Stop-TaskProcessing teardown control flow. Only the file/engine-loader boundary
    # (Get-Module 'DSC.PipelineRunner.Akkodis') is pointed at the repository root so the real
    ...
    BeforeAll {
        ...
        # --- Load the real dependency graph (classes first, then the runner chain and engine seam).
        #
        ...
        # Resource properties resolve <params=Name> tokens before string interpolation, so the
        # parameter-expansion chain is loaded alongside Expand-HashTable.
        ...
        # Point ONLY the engine-file loader (Invoke-Action's Get-Module 'DSC.PipelineRunner.Akkodis')
        # at the repository root so it resolves the real Actions/Engine/DscV2.ps1 on a runner with
        # no installed module. The parameter filter leaves every other Get-Module call untouched --
        # notably the ones DSC makes internally while resolving the resource.
        Mock -CommandName Get-Module -ParameterFilter { $Name -eq 'DSC.PipelineRunner.Akkodis' } -MockWith {
        ...
        # resources that depend on it. Merge-StubResources and Expand-CompositeResources (this
        # repo's own pipeline stages) fall through the default arm, which is correct for fixtures
        # with no stub or composite resources.
    ...
    It "tears the environment down: the Absent Project halts the run via Stop-TaskProcessing" {
        ...
        $teardown = Start-DscRunner -FilePath $script:Configs.Teardown -ConfigurationMode 'ApplyOnly' -DSCCompositeResourcePath $script:CompositeResourcePath -Engine 'DscV2'
