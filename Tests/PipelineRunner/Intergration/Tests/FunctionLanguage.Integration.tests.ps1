Describe "the extended function language through a real runner pass" -Tag Integration, HostedIntegration {
    ...
    #
    #   * the allow-list admitting each new accessor in a condition read from a file
    #   * `variables()` feeding them what the real Set-Variables wrote
    #   * `using()` gated by the real notify map, from a preCondition rather than
    #     from property expansion
    #   * `nodeName()` / `configurationFile()` surviving the zero-argument rewrite and
    #     being CLEARED when the file is finished
    ...
    # scripts under "Pipeline Rules/Custom", so notify expansion and the topological sort are
    # genuinely exercised. This repo's class-based configuration parsing chain
    # (DSCConfigurationFile/DSC_Resource/.../ConvertTo-PipelineTask) is dot-sourced for real below,
    # so [DSCConfigurationFile]::New() inside Start-DscRunner genuinely parses the JSON fixtures
    ...
    BeforeAll {
        ...
        # Start-DscRunner clears and repopulates all three per run, so one set here is safe
        ...
        # Every test in this file drives the real Start-DscRunner; wrapping the two mandatory
        # parameters this repo added on top of Dsc.PipelineRunner's own signature
        # (-ConfigurationMode / -DSCCompositeResourcePath) here keeps every call below focused on
        # what upstream actually varies: -FilePath and -EngineAction.
        function Invoke-TestRunner {
            param(
                [Parameter(Mandatory)] [string] $FilePath,
                [Parameter(Mandatory)] [scriptblock] $EngineAction,
                [switch] $IgnoreErrors,
                # This repo's Start-DscRunner sets $script:StopTaskProcessing on an unhandled
                # preCondition/property-resolution failure unless -ContinueOnError is passed,
                # halting every subsequent resource in the file rather than scoping the failure
                # to the offending resource alone. Tests that assert a SIBLING resource still
                # runs after one resource's condition is rejected pass this switch through.
                [switch] $ContinueOnFailure
            )
            $runnerArgs = @{
                FilePath                  = $FilePath
                ConfigurationMode         = 'ApplyOnly'
                DSCCompositeResourcePath  = $script:CompositeResourcePath
                EngineAction              = $EngineAction
            }
            if ($IgnoreErrors) { $runnerArgs.ErrorAction = 'SilentlyContinue' }
            if ($ContinueOnFailure) { $runnerArgs.ContinueOnError = $true }
            return Start-DscRunner @runnerArgs
        }
    }
...
'@
            ...
            $result = Invoke-TestRunner -FilePath $config -EngineAction (New-RecordingEngine -Calls $calls)
...
'@
            ...
            $result = Invoke-TestRunner -FilePath $config -EngineAction (New-RecordingEngine -Calls $calls) -IgnoreErrors -ContinueOnFailure
...
'@
            ...
            $result = Invoke-TestRunner -FilePath $config -EngineAction $engine
...
'@
            ...
            $result = Invoke-TestRunner -FilePath $config -EngineAction $engine -IgnoreErrors
...
'@
            ...
            Invoke-TestRunner -FilePath $config -EngineAction (New-RecordingEngine -Calls $calls) | Out-Null
...
'@
            ...
            $result = Invoke-TestRunner -FilePath $config -EngineAction (New-RecordingEngine -Calls $calls) -IgnoreErrors
