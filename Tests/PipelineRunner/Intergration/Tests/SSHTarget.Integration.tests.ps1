...
Describe "Target/SSH against a live sshd with a PowerShell subsystem" -Tag Integration, HostedIntegration {
    ...
    # Target/SSH.ps1's own header records that it is only unit-tested with New-PSSession mocked.
    # This suite is that validation. It runs on an agent with openssh-server installed, 'Subsystem
    # powershell' registered in sshd_config and the agent user's own key authorised, then connects
    # over SSH to the agent itself. That needs no second machine and is still a real remote
    # connection: the session is built by the real New-PSSession over the real ssh client, and
    # serviced by a separate pwsh process started by sshd.
    #
