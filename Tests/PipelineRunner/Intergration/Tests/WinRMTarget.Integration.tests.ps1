...
Describe "Target/WinRM against a live WinRM listener" -Tag Integration, RemotingSelfHosted {
    ...
    # runner and connects over WinRM to the runner itself, so it needs no second machine while
    # still going through the genuine WSMan stack: a loopback WinRM connection is a real remote
    # connection as far as CIM/PowerShell remoting is concerned - the sessions are built by the
    # real New-CimSession/New-PSSession, carried over HTTP(S) to the WinRM service, and serviced
    # by a separate wsmprovhost process.
    #
    ...
    #     shipped engine action (Actions/Engine/DscV2.ps1)
    #   * a PSSession from this action carries Invoke-Command, which is how the DSC v3 engine
    #     reaches the far side (Actions/Engine/DscV3.ps1)
    ...
    It "drives the DscV2 engine action through a real remote session" -Skip:(-not $script:WinRMAvailable) {
        ...
        #
        # PSDesiredStateConfiguration 2.x, which PowerShell 7 uses, removed the -CimSession
        # parameter from Invoke-DscResource, so a remote DSC v2 evaluation has to be carried
        # over the PSSession instead - which is what Actions/Engine/DscV2.ps1 does.
