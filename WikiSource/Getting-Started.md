...
```powershell
Install-Module DSC.PipelineRunner.Akkodis -Scope CurrentUser
...
- [Composite and Stub Resources](Composite-and-Stub-Resources) — merging partial resources
  declared at a different layer, and expanding a resource into another whole configuration file.
- [Scheduled Enforcement and Execution Overrides](Scheduled-Enforcement) — restricting `Set` to a
  maintenance window, and forcing one resource's mode regardless of the run's.
