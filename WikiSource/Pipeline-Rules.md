...
│   ├── Merge-StubResources.ps1
│   ├── Expand-CompositeResources.ps1
...
`Merge-StubResources` and `Expand-CompositeResources` are new in this fork — see
[Composite and Stub Resources](Composite-and-Stub-Resources) for what each one does. Neither has
an upstream equivalent; upstream's `Custom/` directory has only the two ordering tasks below.

## When they run

```
Parse the compiled file
 ├─ Custom:   Merge-StubResources         → fold merge_with stubs into their targets
 ├─ Custom:   Expand-CompositeResources   → splice composite nodes' inner resources in place
...
By the time `Expand-NotifyDependsOn` and the pre-parse rules run, every stub and composite node
has already been resolved into ordinary resources — those two later stages never see a
`merge_with` key or a `composite/...` type.
...
```
[DSC.PipelineRunner.Akkodis] Property [visibilty] does not exist in resource [AzDoProject] in module [AzureDevOpsDscNative]
[DSC.PipelineRunner.Akkodis] Property [Ensure] does not match the selected values in resource [File][Ensure]. Permitted values: Present, Absent
