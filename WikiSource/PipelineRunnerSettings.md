...
Overrides](Scheduled-Enforcement). It is validated separately from `PipelineRunnerSettings` and
resolves to the run's effective mode before any file is compiled.
...
```powershell
$settings = & (Get-Module DSC.PipelineRunner.Akkodis) { Get-PipelineRunnerSetting -ConfigurationDirectory 'C:\config' }
$settings['Engine']
```
