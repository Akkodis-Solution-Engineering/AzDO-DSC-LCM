...
Everything human-readable goes to the information stream tagged `DSC.PipelineRunner.Akkodis`, never
`Write-Host`, so it can be captured or redirected:

```powershell
Invoke-DscRunner -ConfigurationSourcePath 'C:\config' -InformationVariable log
$log | Where-Object { $_.Tags -contains 'DSC.PipelineRunner.Akkodis' } | ForEach-Object { $_.MessageData }
```
