...
> **Status: design plan, not implemented.** This document was ported from upstream
> `Dsc.PipelineRunner` (`docs/dsc-v3-config-functions.md`) as part of importing that project's
> documentation into `DSC.PipelineRunner.Akkodis`. It was an unimplemented plan there and it
> remains unimplemented here — nothing under `source/` or `Actions/` in this repo has a
> `ConfigFunctionMode` setting, a `Test-DscV3FunctionExpression` helper, or a
> `DscV3Document` engine. It is kept as a design record / forward-looking proposal, not a
> description of current behavior.

This covers two related questions raised against the DSC v3 work already merged into this
repo (`Actions/Engine/DscV3.ps1`, `ConvertTo-DscV3ConfigurationDocument` — see
[`docs/dsc-v3.md`](dsc-v3.md)):

1. How should `DSC.PipelineRunner.Akkodis` parse and handle **DSC v3 configuration functions**
...
The runner already has a configuration-function mechanism, unrelated to DSC v3 support.
`Start-DscRunner.ps1` resolves each resource's `properties` in two passes (`Expand-Parameters`
then `Expand-HashTable`):

- `Expand-Parameters` does whole-scalar substitution of `<params=Name>` tokens, preserving the
  parameter's native type.
- `Expand-HashTable` then runs `$ExecutionContext.InvokeCommand.ExpandString(...)` over every
  remaining string, which expands `$variable` references and `$(...)` sub-expressions.

The runner's function surface, callable from inside a `$(...)` sub-expression, a `preCondition`,
or a `postCondition`, is considerably larger today than it was when this plan was first written
upstream — it now includes lookups (`parameters()`, `variables()`, `reference()`, `using()`),
run context (`nodeName()`, `configurationFile()`), logic (`equals()`, `not()`), string/collection
helpers (`concat()`, `empty()`, `coalesce()`, `toLower()`, `toUpper()`, `startsWith()`,
`contains()`), arithmetic (`add()`, `sub()`, `mul()`, `div()`, `mod()`, `min()`, `max()`,
`int()`, `float()`), and the post-condition-only `result()`/`stopProcessing()`. See
[WikiSource/Function-Language.md](../WikiSource/Function-Language.md) for the authoritative,
current list — this plan's own examples below only use the original five
(`parameters`/`variables`/`reference`/`equals`/`not`) since those are what the underlying
question is about.

`preCondition`/`postCondition` are evaluated separately, as a `[scriptblock]`, gated by
`Assert-SafeConditionExpression` (`source/Private/Runner/Assert-SafeConditionExpression.ps1`) so
they stay side-effect-free predicates (`postCondition` additionally allow-lists `result()` and
`stopProcessing()`).

This is the runner's own ARM-Template-like function language: property values look like
`$(parameters('DriveLetter'))`, `$(reference('Disk1'))`, or a `preCondition` of
...
see Microsoft's
[DSC configuration functions](https://learn.microsoft.com/powershell/dsc/concepts/enhanced-authoring)
...
- **`[resourceId('type','name')]`** and DSC v3's own `reference()` (which resolves against a
  ...
  `reference()`, but resolved by `dsc.exe`'s document-scoped resolver, not the runner's own
  `$references`.
...
- The two languages are **not interchangeable syntax** — `$(...)` PowerShell sub-expressions vs.
  ...
  `Expand-HashTable` completely untouched (it isn't a `$(...)` expression, so `ExpandString` is a
  no-op on it) and is handed to the engine **literally**, as the string `"[envvar('X')]"`. For
  the `DscV2` engine that is simply wrong (the literal string reaches `Invoke-DscResource`). For
  the `DscV3` engine, `dsc resource test|set|get --input <json>` does **not** evaluate
  configuration functions either — that resolver only runs inside `dsc config ...`, which the
  runner does not call (see §4). So today, DSC v3 functions in a compiled configuration silently
  do nothing useful under either engine in this repo. This is the gap this plan closes.
- `parameters()`/`variables()`/`reference()` mean different things depending on which language is
  in play. The plan below must not let a `[parameters('X')]` token be silently misread as the
...
`Target` / `Credential` selection keys documented in
[WikiSource/PipelineRunnerSettings.md](../WikiSource/PipelineRunnerSettings.md)), with two
values:
...
configuration bug (a `[resourceId(...)]` left in a per-resource `DscV3` run) behind
...
- `preCondition`/`postCondition` (`Assert-SafeConditionExpression` + scriptblock),
- `preExecutionScript`/`postExecutionScript`,
- `notify`/`dependsOn` ordering (`Expand-NotifyDependsOn` + `Sort-DependsOn`).
...
document from compiled resources. But `Start-DscRunner` never calls `dsc config` — the `DscV3`
engine (`Actions/Engine/DscV3.ps1`) only calls `dsc resource test|set|get` **once per resource**,
with that resource's `properties` as `--input`. That path cannot run DSC v3's function resolver,
and it cannot resolve `[resourceId(...)]` / native `reference()` against sibling resources,
because `dsc resource` has no document context — it only ever sees one resource in isolation.
...
- **`Actions/Engine/DscV3Document.ps1`** (name tentative) — instead of dispatching Test/Set/Get
  ...
  engine contract documented in [WikiSource/Engines.md](../WikiSource/Engines.md#the-engine-contract),
  so it cannot be a drop-in `Actions/Engine/*` file under the current typed seam without
  extending that seam — see open question below). It:
  1. builds the full document via `ConvertTo-DscV3ConfigurationDocument` from the resources that
     survived the runner's own `preCondition`/`dependsOn` pre-processing (§3's "what the runner
     still owns"),
  ...
  3. parses the per-resource results out of `dsc config`'s JSON response and maps each back onto
     the runner's existing per-resource report record, preserving today's reporting shape.

Trade-offs to call out explicitly in the eventual implementation PR:

- **Per-resource `preCondition` still has to be evaluated by the runner before building the
  document** (dsc.exe has no equivalent), so a conditionally-skipped resource must be filtered
  out of the document, not left in with a false condition — DSC v3 has no per-resource skip
  semantics to map onto.
...
- **`$references`/notify `$resourceOutputs` population**: today `Start-DscRunner` records each
  resource's `Get` output as it goes, so a *later* resource's runner-level `reference()`/
  `using()` call can see an *earlier* resource's state within the same file. A whole-document
  apply produces all results at once, after the fact — fine for DSC v3's own native
  `[reference(...)]`/`[resourceId(...)]`, which resolve inside `dsc.exe`'s own pass, but it means
  the runner's own `reference()`/`using()` functions are unavailable to a property being
  evaluated in `Native` mode for that resource. This is an acceptable, documented restriction:
...
**DSC v2** — `Invoke-DscResource` (`PSDesiredStateConfiguration`) is a per-resource cmdlet; it
has no "apply this whole document" entry point. The only DSC v2 mechanism that applies a
*combined* configuration is the classic MOF pipeline — compile a `Configuration` block to a
`.mof` and run it through `Start-DscConfiguration` against a Local Configuration Manager (LCM).
That surface is out of scope for this project by design ("no LCM"). **Conclusion: DSC v2 has no
combined-apply path and is not expected to gain one.** It stays resource-by-resource via
...
per-resource reporting, full support for the runner's own `preCondition`/`postExecutionScript`/
`reference()`/`using()`), and whole-document apply is the opt-in path a configuration reaches for
only when it actually uses DSC v3 native functions.
...
3. Extend the engine seam to accommodate a once-per-file engine shape (or introduce
   ...
   completely unchanged). Needs a design decision, not just an implementation — see open
   questions below.
4. `Actions/Engine/DscV3Document.ps1`: build filtered document, call `dsc config`, map results
   back to the runner's report shape.
5. Docs: extend [`docs/dsc-v3.md`](dsc-v3.md) with a "Configuration functions" section covering
   `Native` mode, its restrictions (§4's trade-offs), and a worked example; CI proof exercising a
   document that actually contains a native function (`envvar()` is the easiest to prove
   deterministically in CI).

## 7. Open questions for the maintainer

- **Engine contract shape for whole-document apply.** The existing typed engine contract
  (`{Method, ModuleName, Name, Property} → [DscMethodResult]`, documented in
  [WikiSource/Engines.md](../WikiSource/Engines.md#the-engine-contract)) is called once per
  resource per method. A whole-document engine is called once per *file* per method and returns
  *N* results. Cleanest options: (a) keep `Actions/Engine/*` exclusively per-resource and have
  `Start-DscRunner` special-case a `DscV3Document`-selected engine with an entirely separate call
  path (simplest, but the once-per-file behavior lives outside the `Actions/` loader, breaking
  the "every engine is a drop-in action" property this repo has otherwise held to); (b) widen the
  typed contract so an engine can declare `-SupportsDocumentApply` and receive
  `{Method, Resources[]}` instead, returning `[DscMethodResult[]]` (keeps everything inside the
  loader, but is a breaking change to the seam every existing/custom engine relies on). Recommend
  (a) for a first cut, with (b) revisited only if a second whole-document engine shows up and the
...
- **Should `Native` mode support DSC v2 at all**, e.g. by having the runner itself implement
  ...
  function semantics that could drift from Microsoft's, for a combination (DSC v3 syntax + DSC v2
  execution) that has no real-world motivation.
- **Escaping (`[[`) round-trip through YAML.** Datum/YAML already treats a leading `[` specially
  in flow-sequence position; needs a concrete test fixture proving a scalar string value of
  exactly `"[[literal]"` survives `ConvertFrom-Yaml` as a string and is unescaped correctly,
  before relying on the detection regex in §3.

## See also

- [docs/dsc-v3.md](dsc-v3.md) — the currently-implemented DSC v3 engine and
  `ConvertTo-DscV3ConfigurationDocument`.
- [WikiSource/Function-Language.md](../WikiSource/Function-Language.md) — the runner's own,
  currently-implemented function language.
