<#
.SYNOPSIS
Compiles the GitHub Wiki content for DSC.PipelineRunner.Akkodis into a publishable folder.
...
the Publish_Wiki_Content build task (.build/tasks/5.Build-Wiki.ps1) reads from.
...
$manifestPath   = [System.IO.Path]::Combine($SourcePath, 'source', 'DSC.PipelineRunner.Akkodis.psd1')
...
$null = $indexMarkdown.AppendLine()
$null = $indexMarkdown.AppendLine('Every command exported by `DSC.PipelineRunner.Akkodis`. These pages are generated from each')
...
$null = $sidebar.AppendLine('### DSC.PipelineRunner.Akkodis')
$null = $sidebar.AppendLine()
...
---
DSC.PipelineRunner.Akkodis $ModuleVersion — generated from
[the repository](https://github.com/Akkodis-Solution-Engineering/AzDO-DSC-LCM) on $([datetime]::UtcNow.ToString('yyyy-MM-dd')).
