...
function Assert-SafeConditionExpression {
    ...
    if ($parseErrors -and $parseErrors.Count -gt 0) {
        throw "[DSC.PipelineRunner.Akkodis] Invalid 'condition' expression [$Expression]: $($parseErrors[0].Message)"
    }
    ...
    if ($forbidden) {
        ...
        throw "[DSC.PipelineRunner.Akkodis] A 'condition' must be a side-effect-free predicate. Rejected condition [$Expression] because it contains a $kind [$($offender.Extent.Text)]. Conditions may only call $($allowedCommands -join ', '); they may not invoke other commands, assign variables, or call methods."
    }
}
